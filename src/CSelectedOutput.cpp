...
VRESULT CSelectedOutput::Get(int nRow, int nCol, VAR* pVAR)const
{
	...
	{
		pVAR->type = TT_ERROR;
		pVAR->u.vresult = VR_INVALIDROW;
		return pVAR->u.vresult;
	}
	if ((size_t)nCol >= this->GetColCount() || nCol < 0)
	{
		pVAR->type = TT_ERROR;
		pVAR->u.vresult = VR_INVALIDCOL;
		return pVAR->u.vresult;
	}
	if (nRow)
	{
	...
	std::string &strings)
{
	...
	for (size_t i = 0; i < ncols; i++)
	{
		...
		longs.push_back((long) strlen(this->m_vecVarHeadings[i].u.sVal));
		strings.append(this->m_vecVarHeadings[i].u.sVal);
	}
	...
	for (size_t j = 0; j < ncols; j++)
	{
		for (size_t i = row_number; i < (size_t)(row_number + 1); i++)
		{
			...
			case TT_EMPTY:
				break;
			case TT_ERROR:
				longs.push_back(m_arrayVar[j][i].u.vresult);
				break;
			case TT_LONG:
				longs.push_back(m_arrayVar[j][i].u.lVal);
				break;
			case TT_DOUBLE:
				doubles.push_back(m_arrayVar[j][i].u.dVal);
				break;
			case TT_STRING:
				longs.push_back((long) strlen(m_arrayVar[j][i].u.sVal));
				strings.append(m_arrayVar[j][i].u.sVal);
				break;
			...
			case TT_ERROR:
				{
					...
					v.Clear();
					v.type = TT_ERROR;
					v.u.vresult = (VRESULT) longs[i_longs++];
					this->PushBack(headings[j].c_str(), v);
				}
				break;
			case TT_LONG:
	...
	for (size_t j = 0; j < (size_t)ncol; j++)
	{
		for (size_t i = 0; i < (size_t)nrow; i++)
		{
			...
			case TT_ERROR:
				...
				break;
			case TT_LONG:
				doubles.push_back((double) m_arrayVar[j][i].u.lVal);
				break;
			case TT_DOUBLE:
				doubles.push_back(m_arrayVar[j][i].u.dVal);
				break;
